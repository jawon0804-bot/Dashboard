...
// ⚠️ datetime은 "2026-07-27 14:30:22"(공백 구분) 형식을 전제로 한다.
//    구분자 제거 정규식을 m-event 쪽 구현과 일부러 동일하게 유지하고 있으므로,
//    포맷을 바꿀 거면 3개 구현을 같이 바꿔야 한다.
function buildPhotoFileName(datetime, facilityId, index) {
  const dt = String(datetime ?? "").replace(/[-:\s]/g, "").slice(0, 12);
  const fid = String(facilityId ?? "").replace(/\s/g, "_");
